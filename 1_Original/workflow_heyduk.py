#OBS thisscript contains code that was not used, see github for the clean version
...
def ML_tree(path_in, genes, path_out):
    """Using IQTREE to find the best model"""
    inputs = [path_in+genes+".fasta", path_in+genes+".part"]
    outputs = [path_out+genes+".part.treefile"]
    options = {'cores': 2, 'memory': "40g", 'walltime': "12:00:00", 'account':"dypsidinae"}
    ...
    iqtree2 -s {genes}.fasta -T AUTO -m TEST  -p {genes}.part -mset mrbayes  -B 1000 --undo

    mv *.part.ckp.gz /home/kris/dypsidinae/4.IQtree
    mv *aligned_noempty.fasta-out_clean.part.best_model.nex /home/kris/dypsidinae/4.IQtree
...
# ########################################################################################################################
# ########################################---- IQTREE ML tree all models ----#############################################
# ########################################################################################################################

def ML_tree_all(path_in, genes, path_out):
    """Using IQTREE to find the best model"""
    inputs = [path_in+genes+".fasta", path_in+genes+".part"]
    outputs = [path_out+genes+".part.treefile"]
    options = {'cores': 2, 'memory': "40g", 'walltime': "12:00:00", 'account':"dypsidinae"}
    ...
    iqtree2 -s {genes}.fasta -T AUTO -m MFP  -p {genes}.part -B 1000 --undo

    mv *.part.ckp.gz /home/kris/dypsidinae/4.IQtree_freerate
    mv *aligned_noempty.fasta-out_clean.part.best_model.nex /home/kris/dypsidinae/4.IQtree_freerate
    mv *.part.iqtree /home/kris/dypsidinae/4.IQtree_freerate
    mv *.part.model.gz /home/kris/dypsidinae/4.IQtree_freerate
    mv *.part.treefile /home/kris/dypsidinae/4.IQtree_freerate
    mv *.fasta-out_clean.part.best_scheme /home/kris/dypsidinae/4.IQtree_freerate
    mv *.fasta-out_clean.part.best_scheme.nex /home/kris/dypsidinae/4.IQtree_freerate
    mv *.fasta-out_clean.part.log /home/kris/dypsidinae/4.IQtree_freerate
    mv *.fasta-out_clean.part.model.gz /home/kris/dypsidinae/4.IQtree_freerate
    """.format(path_in = path_in, genes = genes, path_out=path_out)
...
# ########################################################################################################################
# ###########################---- wAstral gene trees from IQtree freerate model ----#####################################
# ########################################################################################################################

def ML_freerate_wastral(path_in, gene_tree_file,output):
    """Using wAstral to construct ML species trees from the gene tree files"""
    inputs = [path_in + gene_tree_file]
    outputs = [path_in + output]
    options = {'cores': '16', 'memory': "100g", 'walltime': "48:00:00", 'account':"dypsidinae"}
    ...
    spec = """
    cd {path_in}
    /home/kris/ASTER/bin/wastral -t 16 -o astral_ML_freerate.tre {gene_tree_file} 2> astral_ML_freerate_log.out
...
def astral_scoring_ML_freerate(path_in,input, output):
    ...
    java -D"java.library.path=lib/" -jar astral.5.7.8.jar -q {path_in}{input} -t 2 -o {path_in}scored_wastral_ML_freerate.tre -i {path_in}gene_trees_ML_freerate.nex 2>scored_ML_freerate_log.out
...
def MrBayes_one_file(path_in, output):
    ...
    inputs = [path_in]
    outputs = [path_in + output]
    options = {'cores': '2', 'memory': "40g", 'walltime': "48:00:00", 'account':"dypsidinae"}
...
def MrBayes_wastral(path_in, gene_tree_file, output):
    ...
    cd {path_in}
    /home/kris/ASTER/bin/wastral -t 16 -o astral_Bayes.tre {gene_tree_file} 2> astral_Bayes_log.out
...
# #######################################################################################################################
# #####################################---- Scoring MrBayes Astral Tree  ----#############################################
# #########################################################################################################################

#Scoring existing tree with ASTRAL 
def astral_scoring(path_in,input, output):
    """Using Astral to score existing tree"""
    inputs = [path_in+input]
    outputs = [path_in + output]
    options = {'cores': '16', 'memory': "40g", 'walltime': "24:00:00", 'account':"dypsidinae"}
    ...
    java -D"java.library.path=lib/" -jar astral.5.7.8.jar -q {path_in}{input} -t 2 -o {path_in}scored_wastral_Bayes.tre -i {path_in}MB_gene_trees.tre 2>scored_bayes_astral_log.out
    mv scored_wastral_Bayes.tre /home/kris/dypsidinae/2.MrBayes/
    mv scored_bayes_astral_log.out /home/kris/dypsidinae/2.MrBayes/
...
gwf.target_from_template('consensus_tree', consensus_tree(path_in= "/home/kris/dypsidinae/3.Astral/", input= "astral_for_mrbayes.run1.t",  output="astral_for_mrbayes.run1.t.con.tre"))
...
#Running IQTREE for trimmed and aligned files to find ML genetrees with all possibile substitution models
for i in range(0, len(genes)):
   gwf.target_from_template('ML_tree_all_'+str(i), ML_tree_all(genes = genes[i],
                                                    path_in = "/home/kris/dypsidinae/data/Dryad/B_main_analysis/A_alignments/",
                                                    path_out="/home/kris/dypsidinae/4.IQtree_freerate/"))

#Running wASTRAL gene trees MLfreerate
gwf.target_from_template('ML_freerate_wastral', ML_freerate_wastral(path_in = "/home/kris/dypsidinae/4.IQtree_freerate/", gene_tree_file="gene_trees_ML_freerate.nex", output= "astral_ML_freerate.tre"))


#Running wASTRAL
gwf.target_from_template('ML_wastral', ML_wastral(path_in = "/home/kris/dypsidinae/4.IQtree/", gene_tree_file="gene_trees.nex", output= "astral_ML.tre"))


#Finding the consensus gene trees
for i in range(0, len(genes_converged)):
    gwf.target_from_template('MrB_sumt_'+str(i), MrB_sumt(genes = genes_converged[i],
                                                    path_in = "/home/kris/dypsidinae/data/nex_files/",
                                                    path_out= "/home/kris/dypsidinae/2.MrBayes/"))
...
#Running wASTRAL
gwf.target_from_template('MB_wastral', MrBayes_wastral(path_in = "/home/kris/dypsidinae/2.MrBayes/", gene_tree_file="MB_gene_trees.tre", output= "astral_Bayes.tre"))
...
# Running ASTRAL
gwf.target_from_template('astral_scoring', astral_scoring(path_in = "/home/kris/dypsidinae/2.MrBayes/",
