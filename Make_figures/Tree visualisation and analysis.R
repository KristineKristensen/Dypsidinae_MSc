...
################################################################################
#Importing ASTRAL-III scored trees
################################################################################
...
tr2plot$node.label<- ifelse(tr2plot$node.label == "1.00", "1", tr2plot$node.label)
tr2plot$tip.label <-gsub("_"," ",tr2plot$tip.label)


pdf("consensus_large_4.pdf", height=27, width=11)
plot.phylo(tr2plot,use.edge.length = TRUE, show.tip.label = FALSE, x.lim = c(0.002, 0.11), y.lim = c(7, 176))
obj<-get("last_plot.phylo",envir=.PlotPhyloEnv)
text(rep(max(obj$xx[1:Ntip(tr2plot)]),Ntip(tr2plot)),obj$yy[1:Ntip(tr2plot)],
     labels=tr2plot$tip.label,pos=4)
for(i in 1:Ntip(tr2plot)) lines(c(obj$xx[i],max(obj$xx[1:Ntip(tr2plot)])),
                                  rep(obj$yy[i],2),lty="dotted")
nodelabels(tr2plot$node.label,adj = c(0,0.4), bg="transparent", frame="none", cex=0.8)
dev.off()
...
#################################################################################
#plotting tr 5
#Adding terminal branch lengths
...
split.plotTree(tree,splits,file= "Original_ML_freerate_done_.pdf", ftype="i",fn=foo,lwd=1, cex=0.7, oma = c(1, 1, 1, 1), mar = c(1, 1, 1, 1))
...
split.plotTree(tree,splits,file= "large_ML_all_genes_done_.pdf", ftype="i",fn=foo,lwd=1, cex=0.7, oma = c(1, 1, 1, 1), mar = c(1, 1, 1, 1))
...
large_M
ggplot(large_M, aes(x=large_M[,3], y=large_M[,4])) + 
  geom_point()
...
large_M <- cbind(large_M, tr4_NL=tr4plot$node.label)



#difference between LPP and PP (PP - LPP)
...
tr4_inforplot <- drop.tip(tr4_info, c("D._album", "R._elegans", "R._augusta", "I._mirabilis", "I._polymorpha", "I._thalangensis", "I._namsabiensis" ,"L._rupicola"))
EN_tr4 <-  as.numeric(sub(".*EN=(.*?)\\].*", "\\1",tr4_inforplot$node.label))
EN_tr4[is.na(EN_tr4)] <- 0
length(EN_tr4) == tr4plot$Nnode
matched_nodes_4 <- matchNodes(tr4_info, tr4reroot)
EN_tr4 <- round(EN_tr4, 0)
...
split.plotTree<-function(tree,splits=NULL,file=NULL,fn,...){
  ef<-0.037037037037
  if(!is.null(file)) jpeg(file, width = 14*300, height = 15*300, res = 300)
  if(is.null(splits)) splits<-(floor(0.5*Ntip(tree))+0.5)/Ntip(tree)
  S<-matrix(c(0,splits,splits,1+1/Ntip(tree)),length(splits)+1,2)
  S<-cbind(S[,1]+ef*(S[,2]-S[,1]),S[,2]-ef*(S[,2]-S[,1]))
  for (i in nrow(S):1) {
    if (!is.null(file)) {
      file_name <- paste0(file, "_part", i, ".jpg")
    }
    plotTree(tree, ylim = Ntip(tree) * S[i, ], use.edge.length = FALSE, ...)
    fn()
  }
...
foo<-function(){
  ...
  legend(x=0.009*par()$usr[2],
    ...
    text.col = c( "#D55E00","#0072B2","#CC79A7", "#009E73"),
    bty = "n"
  )
  nodelabels(text=nodelabels_for_plot,  bg= "white" ,col = label_colors, frame="none", width = 0.2, adj=c(1.05, -0.1), cex=0.8)
...
node1 <- as.numeric(tree$node.label)
node2 <- as.numeric(tr2_NL)
label_colors <- rep(NA, length(nodelabels_for_plot))

# Assign colors based on conditions
label_colors[node1 < 0.95 & node2 < 0.95] <- "#D55E00"       # Both below
label_colors[node1 >= 0.95 & node2 < 0.95] <- "#0072B2"     # Only tree$node.label above
label_colors[node1 < 0.95 & node2 >= 0.95] <- "#CC79A7"   # Only tr2_NL above
label_colors[node1 >= 0.95 & node2 >= 0.95] <- "#009E73"   # Both above

# Remove labels that are filtered
label_colors[nodelabels_for_plot == ""] <- NA
tree <- compute.brlen(tree, method = "Grafen", power = 0.25) 
terminal_edges <- which(tree$edge[,2] <= length(tree$tip.label))
tree$edge.length[terminal_edges] <- tree$edge.length[terminal_edges]-0.25

split.plotTree <- function(tree, splits = NULL, file_prefix = NULL, fn = function() {}, ...) {
  ef <- 0.037037037037
  n_splits <- if (is.null(splits)) {
    (floor(0.5 * Ntip(tree)) + 0.5) / Ntip(tree)
  } else splits
  
  S <- matrix(c(0, n_splits, n_splits, 1 + 1 / Ntip(tree)), length(n_splits) + 1, 2)
  S <- cbind(S[, 1] + ef * (S[, 2] - S[, 1]), S[, 2] - ef * (S[, 2] - S[, 1]))
  ...
  for (i in nrow(S):1) {
    if (!is.null(file_prefix)) {
      file_name <- paste0(file_prefix, "_part", i, ".jpg")
      jpeg(file_name, width = width_px, height = height_px, res = dpi)
    }
    
    plotTree(tree, ylim = Ntip(tree) * S[i, ], use.edge.length = FALSE, xlim=c(0.03, 0.86),...)
    fn()
    
    if (!is.null(file_prefix)) {
      dev.off()
    } else if (i < nrow(S)) {
      par(ask = TRUE)  # pause if not saving files, for interactive plotting
    }
  }
}

split.plotTree(tree,splits,file_prefix= "large_PP_move_test11_", ftype="i",fn=foo,lwd=1, cex=0.7, oma = c(1, 1, 1, 1), mar = c(1, 1, 1, 1), use.edge.length=T)
...
tr7infoplot <-drop.tip(tr7_info, c("L._rupicola_1", "L._rupicola_2"))
EN_tr7 <-  as.numeric(sub(".*EN=(.*?)\\].*", "\\1",tr7infoplot$node.label))
EN_tr7[is.na(EN_tr7)] <- 0
length(EN_tr7) == tr7plot$Nnode
matched_nodes_7 <- matchNodes(tr7infoplot, tr7plot)
EN_tr7 <- round(EN_tr7, 0)
...
foo<-function(){
  ...
  nodelabels(text=nodelabels_for_plot,  bg="white",col = label_colors,cex=0.9, frame="none", width = 0.2, adj=c(1.05, -0.1)) 
...
node1 <- as.numeric(tree$node.label)
node2 <- as.numeric(tr1_NL)
label_colors <- rep(NA, length(nodelabels_for_plot))

# Assign colors based on conditions
label_colors[node1 < 0.95 & node2 < 0.95] <- "#D55E00"       # Both below
label_colors[node1 >= 0.95 & node2 < 0.95] <- "#0072B2"     #PP above
label_colors[node1 < 0.95 & node2 >= 0.95] <- "#CC79A7"   # LPP above
label_colors[node1 >= 0.95 & node2 >= 0.95] <- "#009E73"   # Both above
...
tree <- compute.brlen(tree, method = "Grafen", power = 0.3) 
terminal_edges <- which(tree$edge[,2] <= length(tree$tip.label))
tree$edge.length[terminal_edges] <- tree$edge.length[terminal_edges]-0.20
...
split.plotTree <- function(tree, splits = NULL, file_prefix = NULL,  fn = function() {}, ...) {
  ...
  for (i in nrow(S):1) {
    ...
    par(mai = c(1, 2, 1, 1), xpd=NA)  
    plotTree(tree, ylim = Ntip(tree) * S[i, ],  use.edge.length = T,cex=0.8, xlim=c(-0.02, 0.32), ...)
    fn()
...
split.plotTree(tree,splits,file_prefix = "PP_move_MrB_Original_test2_", ftype="i",fn=foo,lwd=1, use.edge.length=T,cex=0.8)
...
################################################################################
################### calculate support averages #################################
################################################################################

################################################################################
#calculating genera support averages for the original data set
...
"D._ramentacea" ,  "D._mocquerysiana",  "D._humbertii", "D._cookei",    
 "D._gautieri"  ,  "D._rivularis", "D._pustulata" , "D._subacaulis","D._brevicaulis" , "D._integra",     
...
################################################################################
#Mean averages across genera in trees
...
tre_node_values <- c(n_val_tr3reroot, n_val_tr4reroot, n_val_tr5reroot, n_val_tr6reroot, n_val_tr7reroot, n_val_tr8reroot, n_val_tr9reroot, n_val_tr10reroot)
...
support = matrix(ncol=10, byrow=TRUE)
colnames(support) <- c("Original consensus", "New consensus", "New ML freerate", "New Bayesian gene trees", "New ML converged genes", "Original ML all genes", "original Bayesian gene trees","Original ML converged gene trees", "Original Ml freerate", "New ML all genes" )
Original_consensus <- n_val_tr1reroot
New_consensus <- n_val_tr2reroot
New_ML_freerate <- n_val_tr3reroot
New_Bayesian_gene_trees<- n_val_tr4reroot
New_ML_converged_genes <- n_val_tr5reroot
Original_ML_all_genes <- n_val_tr6reroot
original_Bayesian_gene_trees <- n_val_tr7reroot
Original_ML_converged_genes <- n_val_tr8reroot
Original_Ml_freerate <- n_val_tr9reroot
New_ML_all_genes <- n_val_tr10reroot


Support_information <- list("New consensus"=New_consensus, "New Bayesian"= New_Bayesian_gene_trees, "New ML converged" =New_ML_converged_genes, "New ML all"=New_ML_all_genes, "New ML freerate" = New_ML_freerate, "Original consensus"= Original_consensus,"Original Bayesian" =original_Bayesian_gene_trees, "Original ML converged" = Original_ML_converged_genes , "Original ML all"= Original_ML_all_genes, "Original Ml freerate" =Original_Ml_freerate)
library(dplyr)
support_for_plot <- bind_rows(lapply(names(Support_information), function(n) {
  data.frame(Group=n, Value= Support_information[[n]])
}))                            
...
means_df <- data %>%
  ...
  mutate(x = as.numeric(factor(Group)))  # get numeric x-position

medians_df <- data %>%
  group_by(Group) %>%
  summarize(median_value = median(Value, na.rm = TRUE)) %>%
  mutate(x = as.numeric(factor(Group)))


ggplot(data, aes(x = Group, y = Value, fill = Group)) +
  geom_boxplot(fill="lightseagreen",alpha = 0.5, outlier.size = 1, width=0.5,outlier.shape = 21, outlier.fill = "lightseagreen", outlier.colour = "black") +
  geom_segment(data = means_df,
               aes(x = x - 0.24, xend = x + 0.25,
 ...
 # Add labels with the mean values
  ...
  # Add labels the median values
  geom_label(data = medians_df,
             aes(x = Group, y = median_value, label = sprintf("%.2f",median_value)),
             vjust = 1.9,
             size = 3,
             color = "white",
             fill="black",
...
################################################################################
########################### Sort LPP values into categories  ###################
################################################################################
#original

tr1plot <-drop.tip(tr1reroot, c("L._rupicola_1", "L._rupicola_2"))
tr6plot <-drop.tip(tr6reroot, c("L._rupicola_1", "L._rupicola_2"))
tr7plot <-drop.tip(tr7reroot, c("L._rupicola_1", "L._rupicola_2"))
tr8plot <-drop.tip(tr8reroot, c("L._rupicola_1", "L._rupicola_2"))
tr9plot <-drop.tip(tr9reroot, c("L._rupicola_1", "L._rupicola_2"))

#Check the tip names
tr7plot$tip.label <-gsub("_"," ",tr7plot$tip.label)
tr1plot$tip.label <-gsub("_"," ",tr1plot$tip.label)
match(tr1plot$tip.label, tr7plot$tip.label)


mntr6<-matchNodes(tr1plot, tr6plot)
mntr7<-matchNodes(tr1plot, tr7plot)
mntr8<-matchNodes(tr1plot, tr8plot)
mntr9<-matchNodes(tr1plot, tr9plot)


#matrix where the nodes that match tr1 are combined with the correct PP value
mntr7_PP=c()
for(i in 1:nrow(mntr7)){
  if(!is.na(mntr7[i,2])) { mntr7_PP[length(mntr7_PP)+1]= tr7plot$node.label[mntr7[i,2] - Ntip(tr7plot)]
  }else{ mntr7_PP[length(mntr7_PP)+1]= "*"
  }
}
...
for (m in all_models){
  for (s in m){
    print(s)
    if (s >0.95 ){"pp_high_"+m <- s }
    else{ "pp_low_"+m}
  }
  
}

trees_to_compare <- list(
  tr6 = tr6reroot,
  tr7 = tr7reroot,
  tr8 = tr8reroot,
  tr9 = tr9reroot
...
# drop the out group tips and format tiplabels
plots_to_compare <- lapply(trees_to_compare, function(tr) {
  p <- drop.tip(tr, c("L._rupicola_1", "L._rupicola_2"))
...
# MatchNodes
mn_list <- lapply(plots_to_compare, function(t) {
  matchNodes(tr1plot, t)
})
unmatched_counts <- sapply(mn_list, function(mn) {
  sum(is.na(mn[, 2]))
})

#Extracts information in the PP vector
extract_PP <- function(mn_matrix, plot_tree) {
  PP_vec <- character(nrow(mn_matrix))
  for (i in seq_len(nrow(mn_matrix))) {
    if (!is.na(mn_matrix[i, 2])) {
      PP_vec[i] <- plot_tree$node.label[ mn_matrix[i, 2] - Ntip(plot_tree)]
    } else {
      PP_vec[i] <- "NA"
    }
  }
...
PP_list <- mapply(extract_PP, mn = mn_list, plot_tree = plots_to_compare)
node_id <- cbind(mntr6[,1],tr1plot$node.label )
support_df <- as.data.frame(PP_list)
original_M_all <- cbind(node_id,support_df)
column_names <- c("2", "tr6", "tr7", "tr8", "tr9")
numeric_pp <- as.data.frame(lapply(original_M_all[ , column_names], as.numeric))
column_names <- c("X2", "tr6", "tr7", "tr8", "tr9")

counts_over <- colSums(numeric_pp[ , column_names ] >= 0.95, na.rm=T)
counts_under <- colSums(numeric_pp[ , column_names ] < 0.95, na.rm=T) 
 
#make dataframe with the values
compare_PP <- function(consensus_tr, other_tree) {
  PP_both_high <- 0
  PP_consensus_high <- 0
  PP_ML_high <- 0
  PP_low <- 0
  
  for (i in seq_len(nrow(consensus_tr))) {
    cons_val <- consensus_tr[i, 1]
    ml_val <- other_tree[i, 1]
    if (is.na(cons_val) || is.na(ml_val) || is.nan(cons_val) || is.nan(ml_val)) {
      next
    }
    
    if (cons_val >= 0.95 && ml_val >= 0.95) {
      PP_both_high <- PP_both_high + 1
    } else if (cons_val >= 0.95 && ml_val < 0.95) {
      PP_consensus_high <- PP_consensus_high + 1
    } else if (cons_val < 0.95 && ml_val >= 0.95) {
      PP_ML_high <- PP_ML_high + 1
    } else {
      PP_low <- PP_low + 1
    }
  }
  return(data.frame(PP_both_high, PP_consensus_high, PP_ML_high, PP_low))
}

PP_comparison <- lapply(column_names[-1], function(colname) {
  compare_PP(
    consensus_tr = numeric_pp[, column_names[1], drop = FALSE], 
    other_tree = numeric_pp[, colname, drop = FALSE]
  )
})

PP_comparison_df <- do.call(rbind, PP_comparison)
PP_comparison_df$tr <- column_names[-1]

PP_comparison_df <- PP_comparison_df[, c("tr", "PP_both_high", "PP_consensus_high", "PP_ML_high", "PP_low")]
PP_comparison_df$unmatched_nodes <- unmatched_counts[PP_comparison_df$tr]

PP_comparison_df

################################################################################
#New
trees_to_compare_new <- list(
  tr3 = tr3reroot,
  tr4 = tr4reroot,
  tr5 = tr5reroot,
  tr10 = tr10reroot
...
# Match the tip labels, so that node labels can be compared
plots_to_compare_new <- lapply(trees_to_compare_new, function(tr) {
  p <- drop.tip(tr, c("D._album", "R._elegans", "R._augusta", "I._mirabilis", "I._polymorpha", "I._thalangensis", "I._namsabiensis" ,"L._rupicola"))
  p$tip.label <- gsub("_", " ", p$tip.label)
  return(p)
})
tr2plot <- drop.tip(tr2reroot, c("D._album", "R._elegans", "R._augusta", "I._mirabilis", "I._polymorpha", "I._thalangensis", "I._namsabiensis" ,"L._rupicola"))
...
# Match node numbers from the trees to the consensus tree
mn_list_new <- lapply(plots_to_compare_new, function(t) {
  matchNodes(tr2plot, t)
})

unmatched_counts <- sapply(mn_list_new, function(mn) {
  sum(is.na(mn[, 2]))
})


#Extracts information in the PP vector
extract_PP_new <- function(mn_matrix, plot_tree) {
  PP_vec <- character(nrow(mn_matrix))
  for (i in seq_len(nrow(mn_matrix))) {
    if (!is.na(mn_matrix[i, 2])) {
      PP_vec[i] <- plot_tree$node.label[ mn_matrix[i, 2] - Ntip(plot_tree)]
...
PP_list_new <- mapply(extract_PP_new, mn = mn_list_new, plot_tree = plots_to_compare_new)

node_id_new <- cbind(mn_list_new$tr3[,1],tr2plot$node.label )
support_df_new <- as.data.frame(PP_list_new)
new_M_all <- cbind(node_id_new,support_df_new)
column_names_new <- c("2", "tr3", "tr4", "tr5", "tr10")
numeric_pp_new <- as.data.frame(lapply(new_M_all[ , column_names_new], as.numeric))
column_names_new <- c("X2", "tr3", "tr4", "tr5", "tr10")

counts_over_new <- colSums(numeric_pp_new[ , column_names_new ] >= 0.95, na.rm=T)
counts_under_new <- colSums(numeric_pp_new[ , column_names_new ] < 0.95, na.rm=T) 
counts_over_new
counts_under_new


#compare the LPP values to the PP values from the consensus tree
compare_PP <- function(consensus_tr, other_tree) {
  PP_both_high <- 0
  PP_consensus_high <- 0
  PP_ML_high <- 0
...
PP_comparison_new <- lapply(column_names_new[-1], function(colname) {
  compare_PP(consensus_tr = numeric_pp_new[, column_names_new[1], drop = FALSE],
    other_tree = numeric_pp_new[, colname, drop = FALSE] 
  )
})


PP_comparison_df_new <- do.call(rbind, PP_comparison_new)
PP_comparison_df_new$tr <- column_names_new[-1]
PP_comparison_df_new <- PP_comparison_df_new[, c("tr", "PP_both_high", "PP_consensus_high", "PP_ML_high", "PP_low")]
PP_comparison_df_new$unmatched_nodes <- unmatched_counts[PP_comparison_df_new$tr]

PP_comparison_df_new
